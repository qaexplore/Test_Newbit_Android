...
import com.spark.otcbitrade.activity.aboutus.VersionContract;
import com.spark.otcbitrade.activity.aboutus.VersionPresenter;
import com.spark.otcbitrade.activity.login.LoginActivity;
import com.spark.otcbitrade.activity.main.presenter.MainPresenter;
import com.spark.otcbitrade.activity.setting.SettingContact;
import com.spark.otcbitrade.activity.setting.SettingPresenterImpl;
...
import com.spark.otcbitrade.entity.ChatTipEvent;
import com.spark.otcbitrade.entity.Currency;
import com.spark.otcbitrade.entity.ExchangeLoginInfo;
...
import com.spark.otcbitrade.entity.MarketSymbol;
import com.spark.otcbitrade.entity.User;
import com.spark.otcbitrade.entity.Vision;
import com.spark.otcbitrade.entity.VisionEntity;
import com.spark.otcbitrade.event.CheckLoginEvent;
import com.spark.otcbitrade.event.CheckLoginSuccessEvent;
import com.spark.otcbitrade.event.LoginoutWithoutApiEvent;
...
import com.spark.otcbitrade.ui.AppVersionDialog;
import com.spark.otcbitrade.utils.FileUtils;
...
import com.spark.otcbitrade.utils.okhttp.FileCallback;
...
import static com.spark.otcbitrade.factory.HttpUrls.TYPE_AC;
import static com.spark.otcbitrade.factory.HttpUrls.TYPE_OTC;
import static com.spark.otcbitrade.factory.HttpUrls.TYPE_OTC_SYSTEM;
import static com.spark.otcbitrade.factory.HttpUrls.TYPE_UC;
...
public class MainActivity extends BaseTransFragmentActivity implements MainContract.View,
    ...
    private void startTCP() {
        ...
        EventBus.getDefault().post(new SocketMessage(GlobalConstant.CODE_KLINE, ISocket.CMD.SUBSCRIBE_THUMB.getCode(), new Gson().toJson(GlobalConstant.getMAP()).getBytes()));
    }

    @Override
    protected void onDestroy() {
        super.onDestroy();
        EventBus.getDefault().post(new SocketMessage(GlobalConstant.CODE_KLINE, ISocket.CMD.UN_SUBSCRIBE_THUMB.getCode(), new Gson().toJson(GlobalConstant.getMAP()).getBytes()));
...
//            selecte(llWallet, 0);
    ...
    protected void loadData() {
        EventBus.getDefault().post(new SocketMessage(GlobalConstant.CODE_KLINE, ISocket.CMD.ENABLE_SYMBOL.getCode(), new Gson().toJson(GlobalConstant.getMAP()).getBytes()));
        EventBus.getDefault().post(new SocketMessage(GlobalConstant.CODE_KLINE, ISocket.CMD.GET_OVERVIEW_THUMB.getCode(), new Gson().toJson(GlobalConstant.getMAP()).getBytes()));
    }
    ...
    private void getRate() {
        ...
        EventBus.getDefault().post(new SocketMessage(GlobalConstant.CODE_KLINE, ISocket.CMD.USD_CNY_RATE.getCode(), json.getBytes()));
    }
                    ...
                    try {
                        ...
                        if (object.optInt("code") == GlobalConstant.SUCCESS_CODE) {
                            ...
                            if (objs != null) {
                                ...
                                initPager();
                                EventBus.getDefault().post(new SocketMessage(GlobalConstant.CODE_KLINE, ISocket.CMD.GET_ALL_THUMB.getCode(), new Gson().toJson(GlobalConstant.getMAP()).getBytes()));
                            }
                        } else {
            ...
            case 5:
                try {
                    ...
                    if (object.optInt("code") == GlobalConstant.SUCCESS_CODE) {
                        ...
                        EventBus.getDefault().post(new SocketMessage(GlobalConstant.CODE_TRADE, ISocket.CMD.PUSH_REQUEST.getCode(), null));
                    } else {
                        NetCodeUtils.checkedErrorCode((BaseActivity) activity, object.optInt("code"), object.optString("'message"));
                    }
                } catch (Exception e) {
